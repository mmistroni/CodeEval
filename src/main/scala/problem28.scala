...
object problem28 extends App {
  ...
  def lSortFunctional(inputLst:List[List[Symbol]]):List[List[Symbol]] = { inputLst.sortWith(_.size < _.size)
  
  }

  def lSortCustom(inputLst:List[List[Symbol]]):List[List[Symbol]] = {
  
    inputLst.sorted(Ordering.by((_:List[Symbol]).size))
  
  }
  
  /**
  def lSortFreq(inputList:List[List[Symbol]]) = {
  
    val grouped = inputList.groupBy(_.size) grouped.toSeq.sortBy(._2.size).map(item => item._2).flatten.toList
  
  }

  def lSortFreq2(inputList:List[List[Symbol]]) = {

    val mappedList = inputList.foldLeft(MapInt, List[List[Symbol]])( (acc, item) => { 
      val lst:List[List[Symbol]] = item :: acc.getOrElse(item.size, ListList[Symbol]) acc + (item.size -> lst) 
    })

    mappedList.toSeq.sortBy(_._2.size).flatMap(item => item._2).toList
